...
- 固定顺序：只读检查项目 → 锁定必要决策 → 修改 Renovate/CI → 本地验证 → 展示 GitHub Settings 变更清单 → 用户确认 → 浏览器配置 → 完整结果报告。
...
- 先读取当前设置，再按 `references/github-settings.md` 输出写入前确认清单。
- 用户确认后，配置 App 或自托管 Secret、Allow auto-merge、Automatically delete head branches 和默认分支 Ruleset。
...
- 闭环结论：Renovate automerge、CI `verify`、Ruleset required check、Allow auto-merge 和自动删除分支是否全部有效。
